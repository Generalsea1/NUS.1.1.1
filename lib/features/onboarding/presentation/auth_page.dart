...
typedef AuthSignIn = Future<void> Function(String email, String password);
typedef AuthSignUp = Future<bool> Function(String email, String password);
...
class AuthPage extends StatefulWidget {
  ...
  final AuthSignIn? onSignIn;
  final AuthSignUp? onSignUp;
...
class _AuthPageState extends State<AuthPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      if (_registering) {
        final signedIn = await (widget.onSignUp?.call(
        ...
        await (widget.onSignIn?.call(
            ...
            widget.emailAuthRepository.signIn(
  ...
  Future<bool> _defaultSignUp() async {
                ...
                child: Padding(
                  ...
                  child: Form(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('NUS', textAlign: TextAlign.center, style: TextStyle(fontSize: 34, fontWeight: FontWeight.w900, color: scheme.primary, letterSpacing: 1.2)),
                        const SizedBox(height: 8),
                        Text('مدير الاقتصاد الذكي للمنزل', textAlign: TextAlign.center, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
                        const SizedBox(height: 8),
                        Text(
                          _registering
                        ...
                        if (_registering) ...[
                          ...
                          TextFormField(
                            ...
                            validator: (value) => value != _passwordController.text ? 'تأكيد كلمة المرور غير مطابق.' : null,
                          ),
                        ],
                        ...
                        if (_message != null) ...[
                          Container(
                            ...
                            child: Text(_message!, style: TextStyle(color: scheme.onErrorContainer)),
                          ),
                          const SizedBox(height: 14),
                        ],
                        FilledButton(
                        ...
                        const SizedBox(height: 8),
                        TextButton(
                          onPressed: _busy ? null : () => setState(() { _registering = !_registering; _message = null; _confirmController.clear(); }),
                          child: Text(_registering ? 'لديك حساب بالفعل؟ تسجيل الدخول' : 'مستخدم جديد؟ إنشاء حساب'),
                        ),
                        const SizedBox(height: 10),
                        Text(
                          'الدخول بالبريد هو المسار الأساسي هنا. Google OAuth يظل موجودًا في البنية الحالية ولا يُستدعى تلقائيًا.',
                          textAlign: TextAlign.center,
