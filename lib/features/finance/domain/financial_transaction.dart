...
class FinancialTransaction implements DomainEntity {
  factory FinancialTransaction({
    ...
    if (cleanId.isEmpty) {
      throw ArgumentError.value(id, 'id', 'Financial transaction ID must not be empty.');
    }
  ...
  factory FinancialTransaction.fromJson(Map<String, dynamic> json) {
    ...
    if (accountId is! String) {
      throw const FormatException('Financial transaction accountId must be a string.');
    }
    if (amountMinorUnits is! int) {
      throw const FormatException('Financial transaction amountMinorUnits must be an integer.');
    }
    if (currencyCode is! String) {
      throw const FormatException('Financial transaction currencyCode must be a string.');
    }
    if (occurredOn is! String) {
      throw const FormatException('Financial transaction occurredOn must be a string.');
    }
