...
class _AskNusPageState extends State<AskNusPage> {
    ...
    return Scaffold(
      ...
      floatingActionButton: _answer == null
          ...
          : FloatingActionButton.extended(
              onPressed: _loading ? null : () => setState(() => _answer = null),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('اسأل NUS', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)),
                const SizedBox(height: 5),
                const Text('إجابة مالية عملية مبنية على أرقام بيتك الحالية، مع ترتيب الأولويات بدل الكلام العام.'),
              ],
            ),
          ),
        ],
  ...
  Widget _liveContext(BuildContext context, FinancialSnapshot financial) {
    return Card(
      child: Padding(
        ...
        child: Wrap(
          ...
          runSpacing: 10,
          children: [
            _contextMetric('الدخل', '${financial.monthlyIncome} ${financial.currencyCode}', Icons.arrow_downward_rounded),
            _contextMetric('التزامات', '${financial.monthlyObligations} ${financial.currencyCode}', Icons.lock_outline_rounded),
            _contextMetric('المصروف الفعلي', '${financial.actualExpensesMinorUnits} وحدات صغيرة', Icons.payments_outlined),
          ],
        ),
      ),
    );
  ...
  Widget _contextMetric(String title, String value, IconData icon) => Row(
  ...
  Widget _answerCard(BuildContext context, AiInsight answer) {
    final scheme = Theme.of(context).colorScheme;
    return Column(
      children: [
        Card(
              ...
              children: [
                const Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('الخلاصة', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
                  ],
                ),
                const SizedBox(height: 10),
                Text(answer.summary, style: const TextStyle(fontSize: 17, height: 1.55, fontWeight: FontWeight.w700)),
              ],
            ),
          ),
        ),
        if (answer.advice.isNotEmpty) ...[
          const SizedBox(height: 10),
          _sectionCard(context, 'ماذا تفعل الآن؟', answer.advice, Icons.flag_rounded),
        ],
        if (answer.warnings.isNotEmpty) ...[
          const SizedBox(height: 10),
          _sectionCard(context, 'انتبه', answer.warnings, Icons.warning_amber_rounded),
        ],
      ],
    );
  }

  Widget _sectionCard(BuildContext context, String title, List<String> items, IconData icon) => Card(
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(children: [Icon(icon), const SizedBox(width: 8), Text(title, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 17))]),
              const SizedBox(height: 10),
