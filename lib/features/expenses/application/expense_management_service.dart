...
class ExpenseManagementService {
  ...
  Future<Expense> createExpense(Expense expense) async {
    ...
    final actual = _looksLikeUuid(expense.id)
        ...
        : Expense(
            ...
            recurringDefinitionId: expense.recurringDefinitionId,
  ...
  Future<Expense> updateExpense(Expense expense) async {
    ...
    if (expense.id.trim().isEmpty) {
      throw ArgumentError.value(expense.id, 'id', 'Expense ID is required.');
    }
  ...
  Future<RecurringExpenseDefinition> createRecurring(
    RecurringExpenseDefinition definition,
  ) async {
    final actual = _looksLikeUuid(definition.id)
        ? definition
  ...
  Future<RecurringExpenseDefinition> updateRecurring(
    RecurringExpenseDefinition definition,
  ) async {
  ...
  Future<RecurringExpenseDefinition> setRecurringEnabled(
    String userId,
    String id,
    bool enabled,
  ) => _recurring.setEnabled(userId, id, enabled);

  Future<int> monthlyActualTotal({
    required int year,
    required int month,
    required String currencyCode,
  }) async {
    final currency = currencyCode.trim().toUpperCase();
    return (await _expenses.list())
        .where((e) =>
            e.date.year == year &&
            e.date.month == month &&
            e.amount.currencyCode == currency)
        .fold<int>(0, (sum, e) => sum + e.amount.minorUnits);
  }

  Future<Map<String, int>> monthlyActualByCategory({
    required int year,
    required int month,
    required String currencyCode,
  }) async {
    ...
    for (final e in await _expenses.list()) {
      if (e.date.year != year ||
          e.date.month != month ||
          e.amount.currencyCode != currency) continue;
      final category = e.categoryCode ?? 'other';
      totals[category] = (totals[category] ?? 0) + e.amount.minorUnits;
    }
    final keys = totals.keys.toList()..sort();
    return <String, int>{for (final key in keys) key: totals[key]!};
  }

  Future<int> monthlyExpectedRecurringTotal({
    required int year,
    required int month,
    required String currencyCode,
  }) async {
    final currency = currencyCode.trim().toUpperCase();
    return (await _recurring.list())
        .where((d) =>
            d.amount.currencyCode == currency &&
            d.appliesToMonth(year, month))
        .fold<int>(0, (sum, d) => sum + d.normalizedMonthlyAmount);
  }

  void _validateOccurrence(Expense expense) {
    if (expense.expenseType == ExpenseType.recurring &&
        expense.recurringDefinitionId == null) {
      throw ArgumentError.value(
        expense.recurringDefinitionId,
        'recurringDefinitionId',
        'Recurring expenses require a recurring definition ID.',
      );
    }
  }
