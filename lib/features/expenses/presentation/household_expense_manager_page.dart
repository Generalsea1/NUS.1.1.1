...
class HouseholdExpenseManagerPage extends StatefulWidget {
  ...
  @override
  State<HouseholdExpenseManagerPage> createState() =>
      _HouseholdExpenseManagerPageState();
}

class _HouseholdExpenseManagerPageState
    extends State<HouseholdExpenseManagerPage> {
  ...
  static const _essentialKeys = <String>[
    'rent',
    'utilities',
    'food',
    'transport',
  ...
  static const _flexibleKeys = <String>[
    'clothing',
    'maintenance',
    'familyFun',
    'other',
  ...
  final HouseholdBudgetAiProvider _aiProvider =
  ...
  Future<void> _load() async {
    ...
    for (final key in _fields.keys) {
      ...
      _controllers[key]!.text = saved == null || saved == 0 ? '' : '$saved';
    }
    ...
    if (client != null && userId != null) {
      try {
        final row = await client
            ...
            .select(
              'country_code,region,currency_code,household_size,adults,children,housing_type,income_frequency,monthly_income,recurring_debt,emergency_target,budget_snapshot',
            )
            .eq('user_id', userId)
            .maybeSingle();
        ...
        if (row != null) {
          _countryController.text =
              (row['country_code'] as String? ?? '').trim();
          _regionController.text = (row['region'] as String? ?? '').trim();
          _currencyController.text =
              (row['currency_code'] as String? ?? '').trim();
          ...
          final emergencyTarget =
              _safeNonNegativeInt(row['emergency_target']);

          if (monthlyIncome > 0) {
            _controllers['income']!.text = '$monthlyIncome';
          }
          if (recurringDebt > 0) {
            _controllers['debt']!.text = '$recurringDebt';
          }
          if (emergencyTarget > 0) {
            _controllers['savingsTarget']!.text = '$emergencyTarget';
          }
        ...
        // Local values remain usable when cloud profile retrieval is unavailable.
      }
    }
    ...
    try {
      ...
      _actualThisMonth = expenses
          ...
          .fold<int>(
            0,
            (sum, e) => sum + e.amount.minorUnits ~/ 100,
          );
    } catch (_) {
      _actualThisMonth = 0;
    }
  ...
  int _value(String key) {
    return int.tryParse(_controllers[key]!.text.trim()) ?? 0;
  }

  HouseholdBudgetInput _input() {
    return HouseholdBudgetInput(
      ...
      providedFields: _fields.keys
          .where(
            (key) =>
                key != 'income' && _controllers[key]!.text.trim().isNotEmpty,
          )
          .toSet(),
  ...
  HouseholdBudgetPlan _buildPlan() {
    return buildHouseholdBudgetPlan(_input());
  }

  HouseholdBudgetActualComparison _actualComparison(
    HouseholdBudgetPlan plan,
  ) {
    final plannedSpending =
        (plan.plannedTotal - plan.reserve).clamp(0, 0x7fffffffffffffff);
  ...
  Future<bool> _saveProfile() async {
    ...
    if (client == null || userId == null) {
      _showMessage(
        _t(
          'Sign in first to save your household data.',
          'سجّل الدخول الأول علشان نحفظ بيانات البيت على حسابك.',
        ),
      );
      return false;
    }
    ...
    if (country.length != 2 || currency.length < 3 || income <= 0) {
      _showMessage(
        _t(
          'Enter country, currency and your real monthly income.',
          'اكتب الدولة والعملة والدخل الشهري الحقيقي الأول.',
        ),
      );
      return false;
    }
    ...
    try {
      ...
      _showMessage(
        _t(
          'Could not save the household profile right now.',
          'ماقدرتش أحفظ بيانات البيت دلوقتي.',
        ),
      );
      return false;
    } finally {
    ...
    try {
      ...
      setState(() {
        ...
        _aiError = _t(
          ...
          'حصل خطأ غير متوقع في خدمة الذكاء الاصطناعي.',
        );
        _plan = _buildPlan();
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    final scheme = ColorScheme.fromSeed(
      seedColor: const Color(0xFF0B776B),
    ...
    final theme = base.copyWith(
      ...
      scaffoldBackgroundColor:
          dark ? const Color(0xFF081310) : const Color(0xFFF5F2EA),
      ...
      inputDecorationTheme: base.inputDecorationTheme.copyWith(
        filled: true,
        fillColor:
            dark ? const Color(0xFF172722) : const Color(0xFFF0F3F0),
      ...
      child: Directionality(
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            title: Text(
              _t('Household Expenses', 'مصروفات مدير المنزل'),
              ...
              IconButton(
                ...
                onPressed: () {
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) =>
                          AiSettingsPage(isArabic: widget.isArabic),
                    ),
                  );
                },
          ...
          body: SafeArea(
            child: ListView(
              ...
              children: [
                ...
                _sectionCard(
                  title: _t(
                    '1. Essential home costs',
                    '١. الأساسيات التي لا تتأجل',
                  ),
                  subtitle: _t(
                    'Protect these first. The manager works around them.',
                    'دي أول بنود الحماية. مدير البيت بيوزّع الباقي حواليها.',
                  ),
                  keys: _essentialKeys,
                ...
                _sectionCard(
                  ...
                  subtitle: _t(
                    'These are the first categories to trim when money is tight.',
                    'دي أول بنود نقدر نقللها وقت الضغط.',
                  ),
                  keys: _flexibleKeys,
                ...
                const SizedBox(height: 12),
                _sectionCard(
                  title: _t('3. Financial safety', '٣. الأمان المالي'),
                  subtitle: _t(
                    'Set a target for the household reserve.',
                    'حدد هدف احتياطي الأمان للأسرة.',
                  ),
                ...
                FilledButton.icon(
                  onPressed:
                      _aiLoading || _savingProfile ? null : _buildWithAi,
                  icon: _aiLoading || _savingProfile
                      ? const SizedBox(
                          width: 20,
                          height: 20,
                    ...
                    child: Text(
                      _aiLoading
                          ? _t(
                              ...
                              'مدير البيت الذكي بيبني الخطة…',
                            )
                          : _t(
                              'Manage my month with AI',
                              'دبّر لي الشهر بالذكاء الاصطناعي',
                            ),
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [scheme.primary, const Color(0xFF084B44)],
          ),
        ),
        child: Column(
          ...
          children: [
            ...
            Text(
              _t(
                'Make the salary last to the end of the month.',
                'خلّي مرتبك يكفي البيت لآخر الشهر.',
              ),
              style: const TextStyle(
                color: Colors.white,
                fontSize: 25,
            ...
            Text(
              _t(
                'The manager sets priorities, protects essentials and cuts flexible spending before the month gets away from you.',
                'البرنامج بيرتب الأولويات، ويحمي الأساسيات، ويقلل البنود المرنة قبل ما الشهر يفلت منك.',
              ),
              style: TextStyle(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                _sectionIcon(Icons.family_restroom_rounded),
                const SizedBox(width: 10),
                Expanded(
                  child: Text(
                    'بيانات البيت',
                ...
                _pill(
                  _profileSaved ? _t('READY', 'جاهز') : _t('SETUP', 'إعداد'),
                ),
              ],
            ),
            ...
            Text(
              _t(
                'Tell the manager about your real household before asking it to plan.',
                'قول لمدير البيت بياناتك الحقيقية الأول قبل ما تطلب منه يدبّر الشهر.',
              ),
              style: TextStyle(
            ...
            _compactTextField(
              _regionController,
              _t('City / governorate (optional)', 'المدينة / المحافظة (اختياري)'),
              _t('Cairo', 'القاهرة'),
            ...
            Wrap(
              ...
              children: [
                _compactStepper(
                  ...
                  _adults,
                  (value) => setState(() {
                    _adults = value.clamp(1, 20).toInt();
                  }),
                ),
                _compactStepper(
                  _t('Children', 'أطفال'),
                  _children,
                  (value) => setState(() {
                    _children = value.clamp(0, 20).toInt();
                  }),
                ),
              ],
            ),
            const SizedBox(height: 10),
            _dropdownField(
              label: _t('Housing type', 'نوع السكن'),
              value: _housingType,
              icon: Icons.home_work_outlined,
              items: {
                'rent': _t('Rent', 'إيجار'),
                'owned': _t('Owned', 'تمليك'),
                'family': _t('Family home', 'بيت العائلة'),
                'other': _t('Other', 'أخرى'),
              },
            ...
            const SizedBox(height: 10),
            _dropdownField(
              label: _t('Income frequency', 'دورية الدخل'),
              value: _incomeFrequency,
              icon: Icons.event_repeat_rounded,
              items: {
                'monthly': _t('Monthly', 'شهري'),
                'weekly': _t('Weekly', 'أسبوعي'),
                'biweekly': _t('Every two weeks', 'كل أسبوعين'),
                'irregular': _t('Irregular', 'غير منتظم'),
              },
              onChanged: (value) =>
                  setState(() => _incomeFrequency = value),
            ),
            const SizedBox(height: 12),
            OutlinedButton.icon(
              ...
              label: Text(
                _savingProfile
                    ? _t('Saving…', 'جاري الحفظ…')
                    : _t('Save household data', 'احفظ بيانات البيت'),
              ),
            ),
          ],
        ),
  ...
  Widget _actualCard(HouseholdBudgetActualComparison comparison) {
    final scheme = Theme.of(context).colorScheme;
    final tone = comparison.isOverPlan
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(
                  comparison.isOverPlan
                      ? Icons.warning_amber_rounded
                      : Icons.fact_check_outlined,
                ),
                const SizedBox(width: 8),
                Text(
                  _t('Reality check', 'مراجعة الواقع'),
                  style: const TextStyle(
                    fontSize: 17,
                    fontWeight: FontWeight.w900,
                  ),
            ...
            Row(
              children: [
                Expanded(
                  child: _metric(
                    _t('Plan', 'الخطة'),
                    _money(comparison.plannedAmount),
                    Icons.flag_outlined,
                  ),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: _metric(
                    _t('Actual', 'الفعلي'),
                    _money(comparison.actualAmount),
  ...
  Widget _quickSummary(HouseholdBudgetPlan plan) {
    final plannedSpend =
        (plan.plannedTotal - plan.reserve).clamp(0, 0x7fffffffffffffff);
    final remaining =
        (plan.input.monthlyIncome - _actualThisMonth).clamp(0, 0x7fffffffffffffff);
        ...
        child: Wrap(
          ...
          children: [
            _summaryTile(
              ...
              _t('Monthly income', 'الدخل'),
              _money(plan.input.monthlyIncome),
            ),
            _summaryTile(
              Icons.donut_small_outlined,
              _t('Planned spend', 'المخطط'),
              _money(plannedSpend),
            ),
            _summaryTile(
              Icons.savings_outlined,
              _t('Safety reserve', 'الاحتياطي'),
              _money(plan.reserve),
            ),
            _summaryTile(
              Icons.trending_down_rounded,
              _t('Still available', 'المتاح الآن'),
              _money(remaining),
            ),
          ],
        ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            for (final key in keys) _field(key),
          ],
        ),
      ),
    );
    ...
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          hintText: _t('Enter amount', 'اكتب المبلغ'),
          suffixText: currency.isEmpty ? null : currency,
          prefixIcon: Icon(_iconFor(key), size: 20),
        ),
        onChanged: (_) {
          setState(() {
            _aiRecommendation = null;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.auto_awesome_rounded, color: onPrimary),
                const SizedBox(width: 8),
                Expanded(
                  child: Text(
                    _t('AI household decision', 'قرار مدير البيت الذكي'),
                    style: TextStyle(
                      fontSize: 17,
                      fontWeight: FontWeight.w900,
                      color: onPrimary,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              _t(
                'How the manager allocates the month',
                'إزاي مدير البيت بيدبّر الشهر',
              ),
              style: const TextStyle(
                fontSize: 17,
                fontWeight: FontWeight.w900,
            ...
            for (final line in management.lines) ...[
              _managementLine(line, labels[line.priority] ?? ''),
              const SizedBox(height: 10),
            ],
          ],
        ),
  ...
  Widget _managementLine(
    ...
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Icon(_priorityIcon(line.priority), size: 21),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 3),
              Text(
                '$priorityLabel${line.autoAllocated ? ' • ${_t('AI estimate', 'تقدير المدير')}' : ''}',
                style: TextStyle(
                  fontSize: 11,
                  color: Theme.of(context).colorScheme.onSurfaceVariant,
                ),
        ...
        Column(
          ...
          children: [
            ...
            if (line.weeklyCap > 0)
              Text(
                '${_money(line.weeklyCap)} / ${_t('week', 'أسبوع')}',
                style: TextStyle(
                  fontSize: 10,
                  color: Theme.of(context).colorScheme.onSurfaceVariant,
                ),
  ...
  Widget _statusCard(HouseholdBudgetPlan plan) {
    final title = switch (plan.status) {
      BudgetStatus.healthy =>
        _t('The month is manageable', 'الشهر قابل للإدارة'),
      BudgetStatus.tight => _t('The month is tight', 'الشهر محتاج حذر'),
      BudgetStatus.overBudget =>
        _t('The plan is above income', 'الخطة أعلى من الدخل'),
      BudgetStatus.incomplete =>
        _t('More information is needed', 'الخطة محتاجة بيانات أكثر'),
    };
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    _t('Safety reserve', 'احتياطي الأمان'),
                    style: const TextStyle(fontWeight: FontWeight.w800),
                  ),
                ),
                Flexible(
