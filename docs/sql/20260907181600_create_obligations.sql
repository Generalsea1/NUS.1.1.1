...
create index if not exists obligations_user_enabled_idx
  on public.obligations(user_id, enabled);

create unique index if not exists obligations_one_legacy_per_user_idx
...
for each row execute function public.touch_obligation_updated_at();

insert into public.obligations (user_id, name, type, amount, currency_code, frequency, enabled)
select hp.user_id,
       'الالتزامات الأولية من إعداد البيت',
       'legacy',
       legacy_amount,
...
from (
  select user_id, currency_code, recurring_debt, budget_snapshot,
         case
           ...
           when (budget_snapshot->>'initialRecurringObligations') ~ '^[0-9]+$'
             then (budget_snapshot->>'initialRecurringObligations')::bigint
           else 0
...
create trigger household_profiles_seed_legacy_obligation
after insert on public.household_profiles
for each row execute function public.seed_legacy_obligation();
