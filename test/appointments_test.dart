...
void main(){
  ...
  test('completed appointment has no active reminder',() async { final p=FakePort(); final a=make(start:DateTime.now().add(const Duration(hours:2)),reminder:AppointmentReminder.atTime); await AppointmentReminderCoordinator(p).sync(a); final before=p.scheduled.length; final cancelledBefore=p.cancelled.length; await AppointmentReminderCoordinator(p).sync(a.copyWith(status:AppointmentStatus.completed)); expect(before,1); expect(p.scheduled.length,1); expect(p.cancelled.length-cancelledBefore,13); });
