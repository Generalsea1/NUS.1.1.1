...
void main() {
  ...
  testWidgets('unauthenticated user is routed to registration/login', (tester) async {
    ...
    await tester.pumpWidget(_host(
      AuthGate(authRepository: auth, profileRepository: profiles),
    ));
...
AuthenticatedAuthState _authenticatedState() => const AuthenticatedAuthState(
      AuthSession(AuthUser(id: 'u1', email: 'test@example.com')),
    );
...
class FakeAuthRepository implements AuthRepository {
  ...
  Future<void> signOut() async {
    _controller.add(const UnauthenticatedAuthState());
  }
...
class FakeProfileRepository implements HouseholdProfileRepository {
  ...
  @override
  Future<void> save(HouseholdProfile value) async {
    saveCalls++;
