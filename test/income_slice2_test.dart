...
void main() {
  ...
  test('invalid amount and currency are rejected', () {
    final invalidAmount = _source(amount: 0);
    expect(const IncomeSourceValidator().validate(invalidAmount), contains('أكبر من صفر'));
  ...
  group('Income Management UI', () {
    ...
    testWidgets('normal source can be deleted and total updates', (tester) async {
      ...
      await tester.pumpAndSettle();
      await tester.tap(find.byKey(const ValueKey<String>('income-delete-rent')));
      await tester.pumpAndSettle();
  ...
  testWidgets('dashboard preserves Slice 1 monthly income when income source read fails', (tester) async {
    ...
    expect(find.textContaining('الدخل القديم'), findsOneWidget);
  });
}
