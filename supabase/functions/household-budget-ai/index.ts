...
const cleanInput = (body: Record<string, unknown>): BudgetInput => {
  ...
  for (const key of ["income", "rent", "utilities", "food", "transport", "debt", "health", "clothing", "maintenance", "familyFun", "other", "savingsTarget", "actualThisMonth"]) result[key] = numberValue(body[key]);
  return result;
};

const profileDefaults: HouseholdProfile = {
  country_code: "EG",
  region: null,
  currency_code: "EGP",
  household_size: 1,
...
const systemPrompt = (profile: HouseholdProfile) => `أنت مدير المنزل الاقتصادي داخل NUS. مهمتك بناء خطة شهرية واقعية من الدخل الحقيقي للمستخدم والتزاماته والمبالغ التي أدخلها بنفسه وسجل الصرف الفعلي لهذا الشهر.
قواعد إلزامية: لا تخترع الدخل؛ لا تغيّر أي مبلغ أدخله المستخدم؛ القيم التي يتركها المستخدم فارغة فقط هي التي تحتاج اقتراحًا؛ الأولوية للسكن والمرافق والمواصلات والصحة والديون ثم الغذاء والاحتياجات الأساسية ثم الصيانة والملابس والفسحة والمتفرقات؛ راعِ الغلاء دون اختلاق أسعار أو أفراد أسرة غير مذكورين؛ احتفظ باحتياطي فقط عندما يسمح الحساب؛ مجموع البنود التي تقترحها + الاحتياطي لا يتجاوز المتاح؛ actualThisMonth دليل على السلوك الحالي فقط؛ كن محافظًا وعمليًا؛ أعداد صحيحة فقط؛ أعد JSON فقط؛ الرسائل باللهجة المصرية العملية.
ملف الأسرة المحفوظ هو المصدر المرجعي للسياق: الدولة ${profile.country_code}${profile.region ? `، المنطقة ${profile.region}` : ""}؛ العملة ${profile.currency_code}؛ عدد أفراد الأسرة ${profile.household_size}؛ البالغون ${profile.adults}؛ الأطفال ${profile.children}؛ السكن ${profile.housing_type}؛ دورية الدخل ${profile.income_frequency}. لا تفترض أن العملة جنيه مصري إلا إذا كان ملف الأسرة يقول ذلك.`;
...
async function decryptToken(value: string) {
  ...
  const packed = decodeBase64(value); const iv = packed.slice(0, 12); const ciphertext = packed.slice(12);
...
async function encryptToken(value: string) {
  if (!encryptionSecret) throw new Error("AI token encryption is not configured.");
  const key = await deriveKey(encryptionSecret); const iv = crypto.getRandomValues(new Uint8Array(12));
  const ciphertext = new Uint8Array(await crypto.subtle.encrypt({ name: "AES-GCM", iv }, key, new TextEncoder().encode(value)));
  const packed = new Uint8Array(iv.length + ciphertext.length); packed.set(iv, 0); packed.set(ciphertext, iv.length);
...
async function getHouseholdProfile(userId: string): Promise<HouseholdProfile> {
  if (!admin) return profileDefaults;
  const { data, error } = await admin.from("household_profiles").select("country_code,region,currency_code,household_size,adults,children,housing_type,income_frequency").eq("user_id", userId).maybeSingle();
  if (error) throw error;
  if (!data) return profileDefaults;
  return {
    country_code: typeof data.country_code === "string" && data.country_code ? data.country_code : profileDefaults.country_code,
    region: typeof data.region === "string" && data.region.trim() ? data.region.trim() : null,
    currency_code: typeof data.currency_code === "string" && data.currency_code ? data.currency_code : profileDefaults.currency_code,
    household_size: numberValue(data.household_size) || 1,
    adults: numberValue(data.adults) || 1,
    children: numberValue(data.children),
    housing_type: typeof data.housing_type === "string" && data.housing_type ? data.housing_type : profileDefaults.housing_type,
    income_frequency: typeof data.income_frequency === "string" && data.income_frequency ? data.income_frequency : profileDefaults.income_frequency,
  };
}
...
Deno.serve(async (req) => {
  ...
  let profile: HouseholdProfile; try { profile = await getHouseholdProfile(userId); } catch (error) { console.error("Household profile resolution failed", error); return json({ ok: false, error: "تعذر قراءة ملف الأسرة المحفوظ. حاول مرة أخرى." }, 500); }
  ...
  const requestContext = { householdProfile: profile, currency: profile.currency_code, input, providedFields: Array.from(provided), knownMandatory, knownFlexible, availableForMissingCategoriesAndReserve: available, missingCategories: categories.filter((key) => !provided.has(key)) };
  const endpoint = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(gemini.model)}:generateContent`;
  let response: Response;
  try {
    response = await fetch(endpoint, { method: "POST", headers: { Authorization: `Bearer ${gemini.token}`, "Content-Type": "application/json", "x-goog-user-project": googleCloudProjectId! }, body: JSON.stringify({ contents: [{ parts: [{ text: `${systemPrompt(profile)}\n\nبيانات المستخدم:\n${JSON.stringify(requestContext)}` }] }], generationConfig: { temperature: 0.2, responseMimeType: "application/json", responseSchema } }) });
  } catch (error) { console.error("Gemini request failed", error); return json({ ok: false, error: "تعذر الوصول إلى Gemini الآن." }, 502); }
